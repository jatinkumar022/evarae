...
export function ReturnStatusModal({
  ...
  const fetchReturnRequest = useCallback(async () => {
    ...
    setError(null);
    try {
      // First get the return request ID
      const res = await fetch(
        `/api/account/return-requests?orderId=${orderId}&sku=${orderItemSku}`,
        {
          credentials: 'include',
          signal,
        }
      ...
      if (data.returnRequest) {
        // Now fetch full details using the return request ID
        const fullRes = await fetch(
          `/api/account/return-requests/${data.returnRequest._id}`,
          {
            credentials: 'include',
            signal,
          }
        ...
        setReturnRequest(fullData.returnRequest);
      } else {
  ...
  }, [orderId, orderItemSku]);

  useEffect(() => {
    if (isOpen && orderId && orderItemSku) {
      fetchReturnRequest();
    } else {
  ...
  }, [isOpen, orderId, orderItemSku, fetchReturnRequest]);
              ...
              <div className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-white mt-1">
                          ...
                          }).format(returnRequest.orderItem.price * returnRequest.orderItem.quantity)}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700">
                    {RETURN_REASON_LABELS[returnRequest.returnReason] ||
                      returnRequest.returnReason}
                  </p>
                  {returnRequest.note && (
                    <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-600 whitespace-pre-wrap">{returnRequest.note}</p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {(() => {
                      ...
                      const filteredStages = STATUS_STAGES.filter(stage => {
                ...
                {returnRequest.images && returnRequest.images.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-gray-900 dark:text-white mb-3 text-sm sm:text-base">Uploaded Images ({returnRequest.images.length})</h3>
