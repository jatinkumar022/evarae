...
const getCachedRelated = cache(async (slug: string) => {
  await connect();
  ...
  if (current) filter['slug'] = { $ne: (current as unknown as { slug: string }).slug };
...
export async function GET(request: Request, { params }: RouteContext) {
  try {
    ...
    const cacheKey = getCacheKey('product', slug, 'related');
    ...
    const products = await getCachedRelated(slug);
