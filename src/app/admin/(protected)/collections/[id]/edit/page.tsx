...
export default function EditCollectionPage() {
  ...
  useEffect(() => {
    if (currentCollection) {
      reset({
        ...
        sortOrder: currentCollection.sortOrder?.toString() || '0',
      });
    }
  ...
  useEffect(() => {
    if (error) {
      toastApi.error('Error updating collection', error);
      clearError();
    }
  }, [error, clearError]);
  ...
  const onSubmit = async (data: CollectionFormData) => {
    ...
    try {
      ...
      toastApi.success('Collection updated successfully', 'The collection has been updated');
      router.push('/admin/collections');
    } catch (err) {
      console.error('Failed to update collection', err);
      toastApi.error('Failed to update collection', 'Please try again');
    }
  };
